...
const priceTable: number = 1500;
const installments: number = 12;
const valueInstallments: number = Number(
  (priceTable / installments).toFixed(2)
);

if (installments === 1) {
  let payments: number = priceTable - priceTable * (2.5 / 100);
  console.log(
    `Preço do produto: R$${priceTable},00\nPagamento à vista. Desconto de 2.5%\n Total da compra: R$${payments}`
  );
  ...
  let payments: number = priceTable + priceTable * (6 / 100);
  console.log(
    `Preço do produto: R$${priceTable},00\nPagamento feito em x${installments} vezes de R$${valueInstallments}.\n Total da compra: R$${payments},00`
  );
} else if (installments <= 15) {
  let payments: number = priceTable + priceTable * (13 / 100);
  console.log(
    `Preço do produto: R$${priceTable},00\nPagamento feito em x${installments} vezes de R$${valueInstallments}.\n Total da compra: R$${payments},00`
  );
} else {
